...
export default function Home() {
  ...
  // ✅ API-ключ УБРАН из клиентского кода
  // Запросы теперь идут через серверный роут /api/search
  ...
  const handleSearch = async () => {
    ...
    try {
      // Запрос идёт на НАШ сервер, а не напрямую на Кинопоиск
  ...
  return (
    <main style={{
      ...
      <div style={{ textAlign: 'center', padding: '80px 20px 50px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '12px', maxWidth: '700px', margin: '0 auto' }}>
          <input
            type="text"
            ...
            <div key={film.id} style={{
              ...
              {film.poster && film.poster.url ? (
                <img
                  src={film.poster.url}
                  alt={film.name}
                  style={{ width: '90px', height: '130px', borderRadius: '10px', objectFit: 'cover', flexShrink: 0 }}
                />
              ) : (
                <div style={{ width: '90px', height: '130px', borderRadius: '10px', backgroundColor: '#1a1a1a', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '36px', flexShrink: 0 }}>
                  🎬
                </div>
              )}
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px', flexWrap: 'wrap' }}>
                  ...
                  {film.year && (
                    <span style={{ color: '#666', fontSize: '14px', backgroundColor: '#1a1a1a', padding: '2px 8px', borderRadius: '6px' }}>
                      {film.year}
                    </span>
                  )}
                  {film.rating && film.rating.kp > 0 && (
                    <span style={{ background: 'linear-gradient(135deg, #FF4B2B, #FF8E53)', padding: '3px 10px', borderRadius: '6px', fontSize: '14px', fontWeight: '700' }}>
                ...
                {film.description && (
                  <p style={{ color: '#888', margin: '0 0 16px', fontSize: '14px', lineHeight: '1.5' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                  {SERVICES.map((service) => (
                    <a
                      key={service.name}
                      href={service.url + encodeURIComponent(film.name || film.alternativeName || query)}
                      target="_blank"
      ...
      {!searched && (
        <div style={{ textAlign: 'center', padding: '40px 20px', color: '#333' }}>
          ...
          <p style={{ fontSize: '16px' }}>Введи название фильма или сериала чтобы начать поиск</p>
        </div>
      )}
    </main>
  );
